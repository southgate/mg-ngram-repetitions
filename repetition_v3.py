...
#    print str(classement_repetitions(D[0]))
    while not ngram[-1] == [] and N <= longueur_max:
        D.append(dict())
        calcul_ngram(N, L, ngram, tf, D, Global)
        N = N+1
    if ngram[0]:
        print '\nFile ' + file + ': '
...
def calcul_ngram(N, L, NG, tf, Di, Global):
    if NG[-1] == []:
        Di[N-1] = []
        return NG
    else:
...
impression_repetitions(G, 'global.txt')
outfile.close()
